...
{
    ap_filter_t *f = get_ml_filter(handle);
    ...
    case APR_INCOMPLETE:
        while (ml_filter_prefetch(f, APREQ_DEFAULT_READ_BLOCK_SIZE) == APR_INCOMPLETE)
            ;   /*loop*/
    }
...
static apreq_param_t *apache2_body_get(apreq_handle_t *handle, const char *name)
{
    ...
    case APR_EINIT:
        ...
        if (ctx->body_status != APR_INCOMPLETE)
            return NULL;
        ml_filter_prefetch(f, APREQ_DEFAULT_READ_BLOCK_SIZE);
...
static APREQ_MODULE(apache2, MOD_LUAEX_MMN);

APREQ_DECLARE(apreq_handle_t *) apreq_handle_apache2(request_rec *r)
{
    struct apache2_handle *req =
        ap_get_module_config(r->request_config, &luaex_module);

    if (req != NULL) {
        get_ml_filter(&req->handle);
        return &req->handle;
    }

    req = apr_palloc(r->pool, sizeof *req);
    ap_set_module_config(r->request_config, &luaex_module, req);
